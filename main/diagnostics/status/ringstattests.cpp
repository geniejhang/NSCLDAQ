...
public:
  ...
  void tearDown() {
    ZmqObjectFactory::shutdown(); // Get rid of the singleton.
  }
protected:
  void construction();
...
void RingStatTests::construction() {
  ZmqSocket* sock = ZmqObjectFactory::createSocket(ZMQ_PUB);
...
void RingStatTests::startMessage()
{
  ...
  try {
    ...
    delete &sock;
    throw;                      // Assert fails are exceptions.
  }
  delete &sock;
...
void RingStatTests::fixedMessage()
{
  ...
  ZmqSocket& sender(*ZmqObjectFactory::createSocket( ZMQ_PUSH));
  ZmqSocket& receiver(*ZmqObjectFactory::createSocket(ZMQ_PULL));
  try {
