 ...
 * meaning of the CFD data is different for each. The DDASRootHit class 
 ...
 * \endcode
 *
 * It should also be noted that the DDASRootHit can be persistently stored in a 
 ...
 * ddasdumper program creates a TTree filled with DDASRootHit object.
 *
 * This is very close to a ROOT-ized DDASHit, and in fact DDASRootHit objects 
 * can be (and are in the ddasdumper!) copy-constructed from DDASHits. The 
 * classes differ in some key ways:
 * 1. The member data in DDASRootHit is public.
 * 2. Any other differences are handled in the copy-construction.
 */
class DDASRootHit : public TObject {
public:
    ...
    /** alue of trigger source bit(s) for 250 MSPS and 500 MSPS. */
    Int_t cfdtrigsourcebit;
    Int_t cfdfailbit;           //!< Indicates whether the CFD failed.
    ...
    // Extra information which may be present in the channel:
...
public:
    ...
    DDASRootHit(const DDASRootHit& obj) = default;
    /** @brief Default assignment operator. */
    DDASRootHit& operator=(const DDASRootHit& obj) = default;
    /**
     * @brief Copy a DDASHit into a DDASRootHit object.
     * @param hit References the hit to copy.
     * @return Reference to the DDASRootHit object.
     */
    DDASRootHit& operator=(DAQ::DDAS::DDASHit& hit);
    /** @brief Destructor. */
    ~DDASRootHit() {};
    /**
