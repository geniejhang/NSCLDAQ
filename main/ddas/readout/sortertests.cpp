...
public:
    /** @brief Cleanup existing rings and recreate them. */
    void setUp()
	{   
	    // try {
	    ...
	    // m_pTestObject = new DDASSorter(
	    // 	*m_pSourceConsumer, *m_pSinkProducer
	    // 	);
	}

    /** @brief Cleanup all managed objects. */
    void tearDown()
	{
...
// /** @brief Flush the hit manager and read the ring items. */
...
//     *payload++        = moduleType; // Module data (revision, MSPS, bits)
//     for(int i = 0; i < 100; i++) {
...
// 	    CRingItem::getFromRing(*m_pSinkConsumer, all)
// 	    );
...
//     *payload++        = moduleType;
//     for(int i = 0; i < 100; i++) {
// 	// I think this means half in/half out:
...
// /** @brief Process a complete run in one chunk. */
