...
namespace DDASReadout {
    ...
    /**
     * @details
     * The hits are sorted and then merged into the sorted hits. I _think_ my 
     * big-O analysis says that the fastest way to do this is to first sort 
     * each dequeue and then merge them using the minheap algorithm. Those then
     * get insertion merged into the main sorted hits deque.
     * 
     * The idea is that each of the deques in the vector is data from one 
     * module. Note that maintaining module separation makes the module sorting
     * several small sorts rather than, potentially one big sort and I think 
     * that comes out ahead.
     *
     * @note The merging is all done is private methods.
     * @note The dequeues in the vector will be emptied.
	...
	if (m_sortedHits.size() > 1) {
	    return (
		m_flushing || (timeStamp(m_sortedHits.back()) - timeStamp(m_sortedHits.front()) > m_emitWindow));
    ...
    /**
     * @details
     * Throws a logic_error exception if the deque is empty. Normally this 
     * should be called after a call to haveHits returns true.
     */
	...
	if (m_sortedHits.empty()) {
		...
		"CHitManager trying to get hits from an empty sortlist"
		);
	}
...
// Private utility methods.
    ...
    /**
     * @details
     * The merge is done by using the method of min-heaps until only one 
     * dequeue has data at which point in time, all elements of that remaining 
     * queue are just appended to the output deque.
     *
     ...
     *
     * For a simple descriptino of minheap merging see e.g. https://www.geeksforgeeks.org/merge-k-sorted-linked-lists-set-2-using-min-heap/
     *
     * @note This method assumes that on input at least one of the newHits 
     * deques has at least one element.
     * @note Currently we use an std::multimap for the minheap... that may 
     * require costly new/free's. If that turns out to be the case, we can 
     * just do std::sort's on a std::vector of std::pair<double, deque*>.
     * Note that multimap must be used because there may be duplicate 
     * timestamps.
     */
    void
    CHitManager::merge(
    ...
    {
	// The minheap will be a tree that's keyed by the timestamps of the
	// hits at the front of the queues to which element bodies point.
	// Note that items emplaced in the map will be ordered by key.
	...
	for (int i = 0; i < newHits.size(); i++) {
	    if (newHits[i].size() > 0) {
		minheap.emplace(
		    std::make_pair(timeStamp(newHits[i].front()), &(newHits[i]))
		    );
	    }
	}
	...
	while (minheap.size() > 1) {
	    auto earliest = minheap.begin(); // Earliest timestamp.
	    std::deque<ModuleReader::HitInfo>* pQ = earliest->second;
	    result.push_back(pQ->front());
	    pQ->pop_front();
	    minheap.erase(earliest);
        
	    // If pQ has more hits enter it back in the map keyed by the
	    // next timestamp.
	...
	// At this point, the map has only one more element. Put all of the
	// remaining items in that queue into the result:
     ...
     * - If the last element of the output has a timestamp that's less than 
     *   or equal to that of the front of the new hits, we can just append 
     *   the new hits.
     * - Otherwise, pull elements off the back of the existing deque into a 
     *   temporary  dequeue until the timestamp on the back of the existing 
     *   dequeue is greaterthan that of the front of the new elements then use 
     *   std::merge to merge the resulting dequeues into the end of the 
     *   existing deque.
     */
    void
    CHitManager::merge(
	...
	while (
	    !result.empty() && !(lessThan(result.back(), newHits.front()))
	    ) {
	    tempQueue.push_front(result.back()); // This preserves ordering.
	    result.pop_back();
	}
	
	// Merge the list into the end of the temp queue since the back of
	// the result queue has a timestamp that's < the timestamps in the
	// front of temp and newHits:
