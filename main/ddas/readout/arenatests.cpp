...
void ArenaTest::alloc_1() {
    std::unique_ptr<DDASReadout::ReferenceCountedBuffer>
	p(m_pTestObj->allocate(100));
...
void ArenaTest::free_1()
{
    ...
    CPPUNIT_ASSERT_NO_THROW(m_pTestObj->free(p));
}
...
void ArenaTest::free_3()
{
    ...
    CPPUNIT_ASSERT_THROW(m_pTestObj->free(p),std::logic_error);
