...
public:
	...
	{
	    EQMSG(
		"Simple body extracts crate id",
		uint32_t(3), hit.GetCrateID()
		); 
	}
	...
	{
	    EQMSG(
		"Simple body extracts slot id",
		  uint32_t(2), hit.GetSlotID()
		);
	}
	...
	{
	    EQMSG(
		"Simple body extracts channel id",
		uint32_t(1), hit.GetChannelID()
		); 
	}
    ...
    void headerLength_0 ()
	{
		...
		uint32_t(18), hit.GetChannelLengthHeader()
		); 
	}
    ...
    void eventLength_0 ()
	{
		...
		uint32_t(22), hit.GetChannelLength()
		); 
	}
	...
	{
	    EQMSG(
		"Simple body extracts time low",
		uint32_t(63111), hit.GetTimeLow()
		); 
	}
	...
	{
	    EQMSG(
		"Simple body extracts time high",
		uint32_t(10), hit.GetTimeHigh()
		); 
	}
    ...
    void qdcSums_0 ()
	{
	    ...
	    EQMSG("Found all 4 energy sums", expected, hit.GetQDCSums());
	}
    ...
    /** @brief Read in a value for an external clock timestamp. */
    void externalClock_0()
	{
	    uint64_t expected = 0xc0c0d0d0a0a0b0b;
	    EQMSG(
		"Timestamp found with QDC and energy enabled",
		expected, hit.GetExternalTimestamp()
		);
	}

    /** @brief External clock without energy sums or QDC. */
    void externalClock_1()
	{
	    // No QDC or energy sums, external timestamp still present...
	    // header length = 6
	    ...
	    EQMSG(
		"Timestamp extracted when no qdc or energy sum",
		uint64_t(0x00a00a0040302010), hit.GetExternalTimestamp()
		);
	}

    /** @brief External clock with sums but no QDC. */
    void externalClock_2()
	{
	    // Energy sums/baselines present with external timestamp...
	    // header length = 10
	    std::vector<uint32_t> data = { 0x0000000e, 0x0c0c0064, 
	    ...
	    EQMSG(
		"Timestamp extracted when no qdc",
		uint64_t(0x00a00a0040302010), hit.GetExternalTimestamp()
		);
	}

    /** @brief External clock with QDC but no sums. */
    void externalClock_3()
	{
	    ...
	    EQMSG(
		"Timestamp extracted when no energy sum",
		uint64_t(0x00a00a0040302010), hit.GetExternalTimestamp()
		);
	}
