...
CEndCommandSRS::CEndCommandSRS(CTCLInterpreter& rInterp,
			     CEventSegmentSRS *myevseg,
			     CExperiment* pexp) : CEndCommand(rInterp) 
{
    std::cout<<"Simon - initialize END COMMAND"<<std::endl;
...
CEndCommandSRS::~CEndCommandSRS()
{
}

// int CEndCommandSRS::transitionToInactive() 
// {
...
// int CEndCommandSRS::readOutRemainingData() 
// {
...
int
CEndCommandSRS::operator()(CTCLInterpreter& interp,
        std::vector<CTCLObject>& objv)
{
  std::cout<<"Simon - operator END COMMAND"<<std::endl;
  // if (objv.size() != 1) {
  //   return TCL_ERROR;
  // }
  // int status = endRun();
...
int CEndCommandSRS::endRun() 
{
    ...
    if(okToEnd) { 
      ...
      if (nAttemptsMade == nAllowedAttempts) {
        ...
        string result;
        tie(triggerEndStatus, result) = CEndCommandSRS::end();
        //if (deviceEndStatus != TCL_OK || triggerEndStatus != TCL_OK) return TCL_ERROR;
        if (triggerEndStatus != TCL_OK) return TCL_ERROR;
      }
    }
...
int CEndCommandSRS::handleEndRun(Tcl_Event* pEvt, int flags)
{
...
int CEndCommandSRS::handleReadOutRemainingData(Tcl_Event* pEvt, int flags)
{
  // EndEvent* pEnd = reinterpret_cast<EndEvent*>(pEvt);
...
void CEndCommandSRS::rescheduleEndTransition() {
  EndEvent* pEnd = reinterpret_cast<EndEvent*>(Tcl_Alloc(sizeof(EndEvent)));
  pEnd->s_rawEvent.proc = CEndCommandSRS::handleEndRun;
  pEnd->s_thisPtr = this;
  Tcl_QueueEvent(reinterpret_cast<Tcl_Event*>(pEnd), TCL_QUEUE_TAIL);
}

void CEndCommandSRS::rescheduleEndRead() {
  // EndEvent* pEnd = reinterpret_cast<EndEvent*>(Tcl_Alloc(sizeof(EndEvent)));
  // pEnd->s_rawEvent.proc = CEndCommandSRS::handleReadOutRemainingData;
  // pEnd->s_thisPtr = this;
  // Tcl_QueueEvent(reinterpret_cast<Tcl_Event*>(pEnd), TCL_QUEUE_TAIL);
}
