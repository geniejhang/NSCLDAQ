...
{ 
  ...
  if(m_fPacketize) {
    int eventSize =  (p.GetIndex() - size.GetIndex());
    *size = eventSize & 0xffff;	// little endian assumption.
    ++size;
    *size = (eventSize >> 16 ) & 0xffff;
...
CReadOrder::Read(void* pBuf)
{
  ...
  if(m_fPacketize) {
    *pwords++ = nRead & 0xffff;	// little endian assumption.
    *pwords   = (nRead >> 16) & 0xffff;
  }
  if(except.GetCount()) {
    throw except;
  }
