		  ...
		  string& srDate, string& srFrom)
{
  ...
  vector<string> markedForDelete;
  vector<pair<string,string> > scheduledForInsert;
  ...
  for(It = m_CurrAlarm.begin(); It != m_CurrAlarm.end(); It++) {
    if(It->first == m_sExpId) {
      ...
      // We have to do things this way to avoid destroying the
      // validity of the iterator:

      markedForDelete.push_back(It->first);
      scheduledForInsert.push_back(make_pair(m_sExpId, sAlm));
    }
  }
  ...
  for(int i  = 0; i < markedForDelete.size(); i++) {
    It = m_CurrAlarm.find(markedForDelete[i]);
    m_CurrAlarm.erase(It);
  }

  // Insert the new items:

  for (int i =0; i < scheduledForInsert.size(); i++) {
    m_CurrAlarm.insert(scheduledForInsert[i]);;
  }
  ...
  // create a new entry
    ...
    char NewCount[10];		// Room for null too.
