...
static const unsigned int LINK_FORMATION_TIME(1); // Seconds delay after link formation.
...
private:
  ...
  int operator()(int argc,char **argv) {
    ...
    //
    if(OneRun) {
      // Since  link establishment is asynchronous the kludge
      // below is about the best we can do since there's no knowledge
      // on our side about when the link is actually ready to transmit
      // data...especially if data are coming from a remote system.
      // Therefore; we hope that delaying a second or so
      // while the involved spectrodaq really _ought_ to be idle
      // will allow the links to completely form.
      // only then will anybody watching us get the all clear to
      // start the run.
      //
      
      sleep(LINK_FORMATION_TIME);
      ofstream ready(".ready");
      ready << endl;
    }
