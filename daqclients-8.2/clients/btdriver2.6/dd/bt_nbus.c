...
/* EAS A64 */
  ...
  end_bt_kget_io:
    ...
    value &= mask;
    TRC_MSG(BT_TRC_DETAIL | BT_TRC_LIO, (LOG_FMT "Register %d, value 0x%x.\n", 
            LOG_ARG, reg, value));
    FEXIT(value);
    return(value);
...
/* EAS A64 */
    ...
    */
    switch (size) {
      case BT_WIDTH_D8:
        TRC_MSG(BT_TRC_DETAIL | BT_TRC_LIO, (LOG_FMT "D08 Register %d, value 0x%x.\n", 
                LOG_ARG, reg, value));
        btp_put_d8(unit_p, base_p, (bt_data8_t) value);
        break;

      case BT_WIDTH_D16:
        TRC_MSG(BT_TRC_DETAIL | BT_TRC_LIO, (LOG_FMT "D16 Register %d, value 0x%x.\n", 
                LOG_ARG, reg, value));
        btp_put_d16(unit_p, base_p, (bt_data16_t) value);
        break;

      case BT_WIDTH_D32:
        TRC_MSG(BT_TRC_DETAIL | BT_TRC_LIO, (LOG_FMT "D32 Register %d, value 0x%x.\n", 
                LOG_ARG, reg, value));
        if (IS_SET(unit_p->bt_status, BT_NEXT_GEN)) {
            if (reg == BT_RDMA_ADDR) {
               if (IS_CLR(unit_p->bt_status, BT_PCI2PCI)) {
