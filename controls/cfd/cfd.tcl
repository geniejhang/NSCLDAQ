...
#######################
	...
	grid $base.save -in $root	-row 13 -column 2 
        ...
        grid $base.lock -in $root       -row 13 -column 8
...
proc SaveFile {name} {
  ...
  puts $fd "#  The hardware configuration of this module was: "
  puts $fd "#     Name \"$ModuleName\""
  puts $fd "#     ModuleBase [format 0x%x $BaseAddress]"
  puts $fd "#     Crate      $Crate\n\n"
  ...
  for {set i 0} {$i < 16} {incr i} {
      puts $fd "set ChannelOn($i) $mask_arr($i)"
  }
...
proc SaveConfig {} {
  ...
  set filename [tk_getSaveFile  -defaultextension .cfd_settings \
			-filetypes $types \
...
proc ReadConfig {file} {
   ...
   for {set i 0} {$i < 16} {incr i} {
     ...
     set mask_arr($i) 1       ;# Enabled.
   }

   # Inittialize other values to defaults.
  ...
  # set our globals according to the sourced setup file:
   ...
   for {set i 0} {$i < 16} {incr i} {
       catch "set mask_arr($i) $ChannelOn($i)"
   }

  # Create and setup a new state object:

   CFDState::Create $Name
    puts "Setting thresholds. [array names Label]"
   for {set i 0} {$i < 16} {incr i} {
     CFDState::SetThreshold $Name $i $Thresholds($i)
       if {[array names  Label $i] != ""} {
	   .threshold$i config -label "$Label($i) Mv"
       }
   }
...
proc SetConfig {} {
   ...
   set ModuleMap [CFD812::Map $BaseAddress $ModuleName $Crate]
...
AutoSave 10000 failsafe.cfd_settings

#
#  Restore settings 
#   Given a file, the current device settings are restored from the file.
#
proc RestoreSettings {filename} {
    global ModuleName
    global ModuleMap
    global mask_arr
    global MajorityLevel
    ...
    # Defaults are now's values.

    for {set i 0} {$i < 16} {incr i} {
	set Thresholds($i) [CFDState::GetThreshold $ModuleName $i]
	set ChannelOn($i) $mask_arr($i) 
    }


    set WidthLow   [CFDState::GetWidth $ModuleName 0]
    set WidthHigh  [CFDState::GetWidth $ModuleName 1]

    set DeadTimeLow    [CFDState::GetDeadtime $ModuleName 0]
    set DeadTimeHigh   [CFDState::GetDeadtime $ModuleName 1]


    set Majority $MajorityLevel

    puts "Restoring $filename"
    source $filename                 ;# Read the settings file.


    # set the device settings into the state object.

    #  Thresholds and mask.

    for {set i 0} {$i < 16} {incr i} {
	CFDState::SetThreshold $ModuleName $i $Thresholds($i)
	CFD812::SetThreshold $ModuleMap $i $Thresholds($i)
	.threshold$i set [CFDState::GetThreshold $ModuleName $i]
	if {[array names Label $i] != ""} {
	    .threshold$i config -label "$Label($i) mV"
	}

	set mask_arr($i) $ChannelOn($i)
    }
    # Deadtime and width:
   
    # set the state object...

    CFDState::SetDeadtime $ModuleName 0 $DeadTimeLow
    CFDState::SetDeadtime $ModuleName 1 $DeadTimeHigh
    CFDState::SetWidth $ModuleName 0 $WidthLow
    CFDState::SetWidth $ModuleName 1 $WidthHigh

    # Set the physical device...

    CFD812::SetWidth $ModuleMap 0  [WidthToRegister $WidthLow]
    CFD812::SetWidth $ModuleMap 1  [WidthToRegister $WidthHigh]

    CFD812::SetDeadtime $ModuleMap 0 [DeadtimeToRegister $DeadTimeLow]
    CFD812::SetDeadtime $ModuleMap 1 [DeadtimeToRegister $DeadTimeHigh]
    

    # Set the gui.

    .width0 set $WidthLow
    .width1 set $WidthHigh

    .deadtime0 set $DeadTimeLow
    .deadtime1 set $DeadTimeHigh
...
###
#  Prompts for a settings file name, reads the current settings from file
#  and sets the interface accordingly.
#
proc ReadSettings {} {
    set filename [tk_getOpenFile -defaultextension ".cfd_settings" \
