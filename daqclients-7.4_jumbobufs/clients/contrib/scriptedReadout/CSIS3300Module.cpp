...
CSIS3300Module::Read(DAQWordBufferPtr& p)
{
  ...
  // We always put a packet structure around this;
  ...
  *start   = (wordsRead & 0xffff); // Little endian assumption.
  ++start;
  *start   = (wordsRead >> 16) & 0xffff;
...
CConfigurableObject::AddConfigParam(CConfigurationParameter* param)
{
    m_Configuration.push_back(param);
    return Find(param->getSwitch());
