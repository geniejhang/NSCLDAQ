...
namespace NSCLDAQ 
{
  ...
  template<class T>
    DataSource<T>* 
    DataSourceFactory<T>::makeSource(std::string uri, 
                                     std::vector<uint16_t> sample, 
                                     std::vector<uint16_t>exclude)
    {
      DataSource<T> *pSource = 0;

      // Deal with the special case of stdin
      if (uri == std::string("-")) {
        // stdin source
        pSource = new FileDataSource<T>(STDIN_FILENO, exclude);
        ...
        if (parsedURI.getProto() == std::string("file")) {
          ...
          pSource = new FileDataSource<T>(parsedURI, exclude);

        } else if (parsedURI.getProto() == std::string("tcp")) {
          // ringbuffer (local or remote):

          pSource = new RingDataSource<T>(parsedURI, sample, exclude);
