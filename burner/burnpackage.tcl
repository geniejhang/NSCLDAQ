...
namespace eval  SEETFDvd {
    ...
    variable ISOFSroot      "/scratch/$user"; # Where ISO temp data lives.
    ...
    # Purpose:           Deals with broken links in a run directory.
    # Inputs:            top - the top level of the directory subtree that
    #                          will be fixed.
    # Outputs            none
    # Errors:            non
    # Description:       This is called to prepare a run directory subtree
    #                    for marshalling.  All broken links must be resoved.
    #                    The following are the cases we consider:
    #                    Link is not broken             - leave it alone.
    #                    Link is broken, not event file -delete it.
    #                    Link is broken, event file     - attempt to connect
    #                                                   it to a file in 
    #                                                   stageraea/current
    #                                                   else delete it.
    #                    All deletions come with a warning on stdout.

    proc FixupLinks {top} {

	#  Enumerate the links we need to consider using the find command:
	
	set links [exec find $top -type l];  # All links in the dirtree.
	...
	foreach link $links {
		...
		set name [file tail $link]
		if {[regexp {run[0-9]+-[0-9]+.evt} $name]} {
		     # Event file...
		    if {[file exists ~/stagearea/current/$name]} {
			# can relink...
			file delete $link
			exec ln -s [glob ~/stagearea/current/$name] $link
		    } else {
			puts "Deleting broken event file link $link ->$target"
			file delete -force $link
		    }
		} else {
		    puts  "Deleting broken non-event file link $link -> $target"
		    file delete -force $link
		}
	    }
	}
	...
	set SpectrumDir $SourceRoot/../savedspectra
	if {[file exists $SpectrumDir] && ([file type $SpectrumDir] == "directory")} {
	...
	set ScalerDir $SourceRoot/../scalerfiles
	if {[file exists $ScalerDir] && ([file type $ScalerDir] == "directory") } {
	...
	for {set i 1} {$i <= $volume} {incr i} {
	    ...
	    gets stdin
	    incr errors [catch "BurnDVD $ISOFSroot/iso$i.iso"]
	}
	if {$errors} {
	    puts "Since the burn returned errors, we'll leave the isos in place"
	} else {
	    puts "Deleting $ISOFSroot"
	    file delete -force $ISOFSroot;	# Clean up when done too.
	}
    }
