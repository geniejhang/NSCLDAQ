...
class CBufferTranslator;  // forward class declaration

template <class T>
class CTranslatorPointer
{
  CBufferTranslator& m_rUnderlyingBuffer; /*! Reference to the BufferTrans. */
 ...
 public:
  ...
  CTranslatorPointer<T>(CBufferTranslator& Translator) :
    m_rUnderlyingBuffer(Translator),
    m_nOffset(0) { }

  // Constructor which receives an offset
  CTranslatorPointer<T>(CBufferTranslator& Translator, const UInt_t& Offset) :
    m_rUnderlyingBuffer(Translator),
    m_nOffset(Offset*sizeof(T)) { }

  // Constructor which receives a BufferTranslator and a address
  // from which to calculate m_nOffset
  CTranslatorPointer<T>(CBufferTranslator& Translator, Address_t const Addr) :
    m_rUnderlyingBuffer(Translator),
  ...
  template <class U>
    CTranslatorPointer<T>(const CTranslatorPointer<U>&RHS) :
    m_rUnderlyingBuffer(RHS.getBuffer()),
    m_nOffset(RHS.getOffset())
    {}

  // Copy constructor for this
  CTranslatorPointer<T>(const CTranslatorPointer<T>& RHS) :
    m_rUnderlyingBuffer(RHS.m_rUnderlyingBuffer),
    m_nOffset(RHS.m_nOffset) { }

  // Assignment operator=
  CTranslatorPointer<T> operator=(const CTranslatorPointer<T>& RHS)
    {
      m_rUnderlyingBuffer = RHS.m_rUnderlyingBuffer;
      m_nOffset = RHS.m_nOffset;
      return *this;
  ...
  template<class U>
    CTranslatorPointer<T>& operator=(const CTranslatorPointer<U>& rhs)
    {
      m_rUnderlyingBuffer = rhs.getBuffer();
      m_nOffset = rhs.getOffset();
      return *this;
  ...
  */
  inline CTranslatorPointer<T> operator++(Int_t)
    {
      CTranslatorPointer<T> before(*this);
      operator++();
      return before;
    }
  ...
  */
  inline CTranslatorPointer<T>& operator++()
    {
      m_nOffset+=sizeof(T);
      return *this;
    }
  ...
  */
  inline CTranslatorPointer<T> operator+(Int_t Offset)
    {
      CTranslatorPointer<T>  p(m_rUnderlyingBuffer);
  ...
  /*!
    \fn CTranslatorPointer<T>& operator+=(Int_t Offset)
  ...
  */
  ...
  /*!
    \fn bool operator<(const CTranslatorPointer<T>& RHS)
    ...
    \param const CTranslatorPointer<T>& RHS - a reference to a TranslatorPointer
                                             to compare with this
   */
  inline bool operator<(const CTranslatorPointer<T>& RHS)
    {  
  ...
  /*!
    bool operator!=(const CTranslatorPointer<T>& RHS
 ...
 public:
  ...
  CBufferTranslator& getBuffer() const
    {
