...
CBufferFactory::Endian MyEndianess()
{
  ...
  if (x.asArray[0] == 1 ) {
    return CBufferFactory::little;
  }
  
  else {
    return CBufferFactory::big;
  }
}
...
CBufferTranslator*
CBufferFactory::CreateBuffer( Address_t pBuffer, Int_t Signature32 )
{
  if( Signature32 == 0x01020304 ) {
    return new CNonSwappingBufferTranslator(pBuffer);
  } 
  else {
    return new CSwappingBufferTranslator(pBuffer);
  }
}
...
void CSwappingBufferTranslator::GetBlock( const Address_t pItem, 
 ...
 */
uint64_t
CSwappingBufferTranslator::getQuad(uint64_t value)
{
...
void CNonSwappingBufferTranslator::GetBlock( const Address_t pItem, 
					    Int_t size, Int_t iOffset )
{
  UChar_t *pBuff = (UChar_t *) m_pBuffer;
  UChar_t *Item = (UChar_t *) pItem;
...
UChar_t CBufferTranslator::GetByte( UInt_t iOffset )
{
  UChar_t Byte;
...
Short_t CBufferTranslator::GetWord( UInt_t iOffset )
{
  Short_t Word;
...
Long_t CBufferTranslator::GetLongword( UInt_t iOffset )
{
  Long_t Longword;
 ...
 */
uint64_t
CNonSwappingBufferTranslator::getQuad(uint64_t value)
{
    return value;
}
